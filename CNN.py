...
class model(object):
    ...
    def __call__(self,x):
        ...
        with tf.device('/device:GPU:0'):
            ...
            """
            Esta sección es importante de considerar, ya que es la forma en que esta trabajando la red,
            los tensores pasaran por cada capa.
            Se notaba al principio que la red tenia un overfitting muy pronto, por lo que se considero que quizas el número 
            de parametros era muy alto y que podía facilmente esta aprendiendo de la red, se movio esto, pero no se tuvo mejora, 
            por lo que se considero que se estaba aprendiendo las imágenes y no encontraba cada vez rasgos de más alto nivel, 
            por lo que se realizo un cambio en el tamaño de los max pooling para tener una mayor agregación de información y así
            enfocarse en rasgos de alto nivel. Esto ayudo a que se mejorara el desempeño.

            El punto más importante fue la utilización de la normalización del batch, en donde se normalizan los valores de los mapas
            de activación, esto con el fin de tener una distribución más controlada lo que permite a la red tener un mayor enfoque
            en los patrones encontrados y no importantes por su escala, asi mismo una distribución constante permite que el entrenamiento
            sea más veloz. Antes de utilizar esta técnica la exactitud no aumentaba del 30%, utilizandolo se alcanzan los valores de alrededor de 80%.

            Se definio un valor de dropout chico, uno más alto no permitia a la red aprender, o quizás si pero en más épocas.
            """
